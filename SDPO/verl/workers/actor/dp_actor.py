...
def _normalize_prm(prm: torch.Tensor, response_mask: torch.Tensor, mode: str) -> torch.Tensor:
    if mode == "sequence":
        lengths = response_mask.sum(dim=-1, keepdim=True).clamp(min=1.0)
        prm_mean = (prm * response_mask).sum(dim=-1, keepdim=True) / lengths
        prm_centered = (prm - prm_mean) * response_mask
        prm_var = (prm_centered ** 2 * response_mask).sum(dim=-1, keepdim=True) / lengths
        return (prm_centered / prm_var.sqrt().clamp(min=1e-6)) * response_mask

    if mode == "batch":
        valid = response_mask.bool()
        if valid.any():
            prm_valid = prm[valid]
            prm_std = prm_valid.std(unbiased=False).clamp(min=1e-6)
            return ((prm - prm_valid.mean()) / prm_std) * response_mask
        return torch.zeros_like(prm)

    if mode == "sequence_demean":
        lengths = response_mask.sum(dim=-1, keepdim=True).clamp(min=1.0)
        prm_mean = (prm * response_mask).sum(dim=-1, keepdim=True) / lengths
        return (prm - prm_mean) * response_mask
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    distill_topk = (
                        ...
                        if loss_mode == "sdpo" and self_distillation_cfg.full_logit_distillation
                        else None
                    )
                    outputs = self._forward_micro_batch(
                        model_inputs,
                    ...
                    if self_distillation_enabled:
                        ...
                        if loss_mode in SD_GRPO_LOSS_MODES:
                            # JSD PRM on sampled tokens.
                            # SD uses +0.5*(softplus(t-s)-log2); AntiSD flips the sign.
                            valid_sd_mask = response_mask
                            if self_distillation_mask is not None:
                                valid_sd_mask = valid_sd_mask * self_distillation_mask.unsqueeze(-1)
                            prm_sign = 1.0 if loss_mode == "grpo_sd" else -1.0
                            teacher_student_gap = (teacher_log_prob - log_prob).detach() * valid_sd_mask
                            prm_raw = (
                                0.5
                                * prm_sign
                                * (torch.nn.functional.softplus(teacher_student_gap) - math.log(2.0))
                                * valid_sd_mask
                            )
                            normalize_mode = self_distillation_cfg.get("grpo_prm_normalize_mode", "sequence")
                            prm_processed = _normalize_prm(prm_raw, valid_sd_mask, normalize_mode)
                            prm_clip = self_distillation_cfg.get("grpo_prm_clip", 5.0)
                            if prm_clip is not None:
                                prm_processed = prm_processed.clamp(-float(prm_clip), float(prm_clip))
                            prm_lambda = float(self_distillation_cfg.get("grpo_prm_lambda", 0.1))
                            refined_advantages = advantages + prm_lambda * prm_processed
                            ...
                            if valid.any():
                                pg_metrics[f"{loss_mode}/prm_raw_mean"] = prm_raw[valid].mean().item()
                                pg_metrics[f"{loss_mode}/prm_raw_abs_mean"] = prm_raw[valid].abs().mean().item()
                                pg_metrics[f"{loss_mode}/prm_processed_mean"] = prm_processed[valid].mean().item()
                                pg_metrics[f"{loss_mode}/prm_processed_abs_mean"] = prm_processed[valid].abs().mean().item()
                                pg_metrics[f"{loss_mode}/teacher_student_gap_mean"] = teacher_student_gap[valid].mean().item()
                                pg_metrics[f"{loss_mode}/A_grpo_abs_mean"] = advantages[valid].abs().mean().item()
                                pg_metrics[f"{loss_mode}/A_refined_abs_mean"] = refined_advantages[valid].abs().mean().item()
                            else:
                                pg_metrics[f"{loss_mode}/prm_raw_mean"] = 0.0
                                pg_metrics[f"{loss_mode}/prm_raw_abs_mean"] = 0.0
                                pg_metrics[f"{loss_mode}/prm_processed_mean"] = 0.0
                                pg_metrics[f"{loss_mode}/prm_processed_abs_mean"] = 0.0
                                pg_metrics[f"{loss_mode}/teacher_student_gap_mean"] = 0.0
