...
class SelfDistillationConfig(BaseConfig):
    ...
    def __post_init__(self):
        ...
        valid_prm_normalize_modes = ["none", "batch", "sequence", "sequence_demean"]
        if self.grpo_prm_normalize_mode not in valid_prm_normalize_modes:
            raise ValueError(
                "self_distillation.grpo_prm_normalize_mode must be one of "
                f"{valid_prm_normalize_modes}, got {self.grpo_prm_normalize_mode}"
            )
