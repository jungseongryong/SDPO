...
def verify(
    ...
    Uses two-stage verification:
    1. Strict string match on extracted \\boxed{} content
    2. math_verify equivalence check on full strings (handles equivalent expressions)

    Args:
        solution_str: The full solution string to verify
        answer: The ground truth answer
        pause_tokens_index: Indices of pause tokens

    Returns:
        Tuple of (is_correct, extracted_prediction_string)
    """
    correct, pred = is_correct_strict_box(solution_str, answer, pause_tokens_index)
    if pred is None:
        pred = ""

    # try Math-Verify equivalence check on full strings
    # mv_parse extracts \boxed{} content internally, so pass complete strings
    if not correct and pred != "":
        try:
            gold_parsed = mv_parse(answer, parsing_timeout=30)
            pred_parsed = mv_parse(solution_str, parsing_timeout=30)
