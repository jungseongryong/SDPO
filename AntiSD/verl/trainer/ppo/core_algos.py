...
def compute_self_distillation_loss(
    ...
    contrastive_teacher_log_probs: Optional[torch.Tensor] = None,
    contrastive_teacher_topk_log_probs: Optional[torch.Tensor] = None,
    ccir_config: Any = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    ...
    # CCIR: compute contrastive information S_t if enabled (proposal_v1 Section 5.2)
    # S_t = log π_t(y|x,z) - log π_t(y|x',z): positive for x-specific tokens
    ccir_S_t = None
    ccir_S_t_topk = None  # [bs, seq, k] distributional S_t over topk vocab items (centered)
    ccir_S_t_topk_raw = None  # [bs, seq, k] raw (uncentered) for metrics
    ccir_enabled = contrastive_teacher_log_probs is not None and ccir_config is not None and ccir_config.enabled
    if ccir_enabled:
        ccir_S_t = (teacher_log_probs - contrastive_teacher_log_probs).detach()
        if contrastive_teacher_topk_log_probs is not None and teacher_topk_log_probs is not None:
            ccir_S_t_topk_raw = (teacher_topk_log_probs - contrastive_teacher_topk_log_probs).detach()
            # Per-position centering: REINFORCE baseline to remove student-topk selection bias.
            # Raw S_t_topk has systematic negative mean (~-1.3) because student's topk includes
            # ~19 non-sampled tokens where correct-prompt teacher concentrates mass elsewhere.
            # Centering preserves relative x-specificity ranking while making weights mean=1.
            ccir_S_t_topk = ccir_S_t_topk_raw - ccir_S_t_topk_raw.mean(dim=-1, keepdim=True)
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        # CCIR: distributional S_t weighting on topk KL loss
        if ccir_S_t_topk is not None:
            # Pad centered S_t for tail bucket (neutral weight=0 → no CCIR adjustment for tail)
            S_t_weights = ccir_S_t_topk
            if kl_loss.size(-1) > S_t_weights.size(-1):
                S_t_weights = F.pad(S_t_weights, (0, 1), value=0.0)

            # weight = clamp(1 + λ·S_t_centered(v), min=0)
            # Decomposes into KL + distributional REINFORCE with per-position baseline.
            # clamp(min=0) is a safety net for rare tail outliers (~0.007% at 3.8σ).
            ccir_kl_weights = (1.0 + ccir_config.prm_weight * S_t_weights).clamp(min=0.0)
            per_token_loss = (kl_loss * ccir_kl_weights).sum(-1)
        else:
            per_token_loss = kl_loss.sum(-1)
            # CCIR fallback: additive PG term on KL loss (per-token S_t)
            if ccir_S_t is not None:
                per_token_loss = per_token_loss + ccir_config.prm_weight * (-ccir_S_t * student_log_probs)
    else:
        assert self_distillation_config.alpha == 1.0, "Only reverse KL is supported for non-full-logit distillation"
        log_ratio = student_log_probs - teacher_log_probs
        # CCIR: S_t directly added to the advantage (proposal_v1 Section 5.3)
        # Combined advantage = (log π_s - log π_t) - λ·S_t
        # x-specific tokens (S_t > 0) get stronger push toward teacher,
        # generic/shortcut tokens (S_t ≤ 0) get weaker push.
        advantage = log_ratio.detach()
        if ccir_S_t is not None:
            advantage = advantage - ccir_config.prm_weight * ccir_S_t
    ...
    if ccir_S_t is not None:
        ...
        if valid_mask.any():
            metrics["ccir/S_t_mean"] = ccir_S_t[valid_mask].mean().item()
            metrics["ccir/S_t_std"] = ccir_S_t[valid_mask].std().item()
        else:
            metrics["ccir/S_t_mean"] = 0.0
            metrics["ccir/S_t_std"] = 0.0
    ...
    if ccir_S_t_topk is not None:
        ...
        if self_distillation_mask is not None:
            valid_mask = valid_mask & self_distillation_mask.unsqueeze(-1).bool()
        if valid_mask.any():
            # Raw S_t_topk (before centering): tracks selection bias magnitude
            S_t_raw_valid = ccir_S_t_topk_raw[valid_mask]
            metrics["ccir/S_t_topk_raw_mean"] = S_t_raw_valid.mean().item()
            # Centered S_t_topk: what actually drives the weights
            S_t_valid = ccir_S_t_topk[valid_mask]
            metrics["ccir/S_t_topk_mean"] = S_t_valid.mean().item()
            metrics["ccir/S_t_topk_std"] = S_t_valid.std().item()
            # Actual weights used (after centering + clamp)
            kl_weights_valid = (1.0 + ccir_config.prm_weight * S_t_valid).clamp(min=0.0)
            metrics["ccir/kl_weight_mean"] = kl_weights_valid.mean().item()
            metrics["ccir/kl_weight_max"] = kl_weights_valid.max().item()
            metrics["ccir/kl_weight_min"] = kl_weights_valid.min().item()
        else:
            metrics["ccir/S_t_topk_raw_mean"] = 0.0
            metrics["ccir/S_t_topk_mean"] = 0.0
            metrics["ccir/S_t_topk_std"] = 0.0
            metrics["ccir/kl_weight_mean"] = 1.0
            metrics["ccir/kl_weight_max"] = 1.0
            metrics["ccir/kl_weight_min"] = 1.0
