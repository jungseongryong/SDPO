#!/bin/bash
# v33-OT-GRPO-long: GRPO baseline on Olmo-3-7B-Think.
# Aligns with v30 Q4T baseline structure: thinking=true, response=32768, max_model_len=67584.
# All other settings match v32 canonical (token-mean / mbs=32 / dapo-math-17k).
set -e
...
# allenai/Olmo-3-7B-Think — user has it in HF default cache.
# Override MODEL_PATH to a local path if needed.
MODEL_PATH="${MODEL_PATH:-allenai/Olmo-3-7B-Think}"
MODEL_NAME=$(echo "$MODEL_PATH" | tr '/' '-')
EXP_NAME="v33-OT-GRPO-long-rollout32768-eval32768-mbs32-lr1e-6-${MODEL_NAME}"
